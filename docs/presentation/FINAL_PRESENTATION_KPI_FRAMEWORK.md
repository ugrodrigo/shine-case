...
- Across the full 9,088-company comparable base, Start contributes **44.64%**
  of revenue and BTP **26.74%**. This captures both large and small companies.
- Plus contributes **28.56%** of all comparable plan revenue at **2.02x** the
  overall average revenue per company. Business reaches 3.22x but contributes
  only 6.95% of revenue and has just 196 comparable companies.
- As secondary concentration context, the highest-value 20% generate **70.24%**
  of comparable revenue. The raw cumulative sensitivity is 73.93%.
...
- BTP contributes EUR 293.7k, or 26.74%, of all comparable revenue across the
  first three complete post-activation months.
- As a secondary concentration sensitivity, 482 BTP companies are in the
  overall top 20%, generating EUR 233.6k.
...
audit table is `outputs/eda/eda_company_account_health_at_cutoff.csv`.

## KPI hierarchy for the case

### Tier 1: put these on the three slides

#### 1. Full-population segment scale and efficiency

Primary ranking denominator: all **9,088 companies** with three complete
post-activation calendar months. The bar metric is:

> Segment revenue across months 1-3 / all comparable revenue across months 1-3.

Bar color uses average segment revenue per company divided by the overall
average of EUR 120.87. This shows efficiency without removing the long tail.

| Segment | Revenue share | Average revenue/company | Index vs overall |
|---|---:|---:|---:|
| BTP | **26.74%** | EUR 178.85 | **1.48x** |
| Consultant | 15.85% | EUR 109.85 | 0.91x |
| Others | 12.24% | EUR 93.33 | 0.77x |
| Retail | 12.19% | EUR 106.80 | 0.88x |
| Start | **44.64%** | EUR 122.22 | 1.01x |
| Plus | **28.56%** | EUR 243.96 | **2.02x** |
| Free | 19.85% | EUR 60.65 | 0.50x |
| Business | 6.95% | EUR 389.71 | **3.22x** |

Why use it: Start and Free receive credit for meaningful revenue generated by
many smaller companies, while Plus, Business, BTP, and other efficient segments
still stand out through revenue per company. No opaque composite score is used.

#### 2. Comparable mature-company health by segment
...
#### 3. Secondary concentration and high-value protection

The highest-revenue 20% of the comparable base generates **70.24%** of revenue.
Use this to describe concentration and size a targeted protection population,
not to rank all personas or plans.

The distinction matters:

- Start is 44.64% of all comparable revenue but 39.70% of top-20 revenue.
- Plus is 28.56% of all revenue but 33.42% of top-20 revenue.
- Business is 6.95% of all revenue but 9.67% of top-20 revenue.
- The 3,237 Start companies outside the top 20% still generate EUR 184.0k,
  representing 37.52% of Start revenue.

For protection, the cumulative high-value population remains useful: 493 of
2,124 assessable continuously monetized high-value accounts are Watch (23.21%),
and 154 are inactive. The EUR 41.0k Watch gap and EUR 19.24k inactive exposure
remain descriptive, not forecast recoverable revenue.

The source is monthly and cannot split activation-month revenue at the exact
activation date. Ages 1-3 therefore remain the preferred comparable window; an
exact 90-day measure would require daily revenue.
...
- `docs/research/SHINE_MARKET_RESEARCH_AND_STRATEGY.md`: source-linked external pressure test
  covering Shine's model, competitors, BTP market limits, e-invoicing, and the
  revised strategic recommendation.
- `outputs/research/Shine_External_Market_Research_and_Revised_Strategy.docx`: formatted Word
...
- `outputs/eda/eda_company_revenue_momentum_at_cutoff.csv`: company-level
  momentum audit table.
- `outputs/eda/eda_top20_account_health_summary.csv`: high-value health table.
- `outputs/eda/eda_account_health_by_segment.csv`: health by persona and plan.
- `outputs/eda/eda_revenue_momentum_threshold_sensitivity.csv`: 20%/30%/40%
  decline sensitivity with and without the EUR 10 floor.
- `outputs/eda/eda_top20_revenue_momentum_by_segment.csv`: Watch concentration
  by persona and initial plan.
- `outputs/eda/eda_segment_opportunity_ranking.csv`: transparent persona and
  initial-plan ranking for the Page 1 bars.
- `outputs/eda/eda_comparable_account_health_by_segment.csv`: mature-base health
  states for the Page 2 stacked bars.
- `docs/findings/SEGMENT_RANKING_AND_HEALTH_FINDINGS.md`: interpretation of both visuals.
- `outputs/eda/eda_first3_full_month_top20_revenue_summary.csv`: preferred
  three-complete-month concentration result.
- `outputs/eda/eda_first3_full_month_top20_revenue_by_segment.csv`: preferred
  persona, plan, and intersection mix.
- `docs/findings/REVENUE_CONCENTRATION_FINDINGS.md`: concentration and segment evidence.
- `docs/findings/COHORT_STATE_FINDINGS.md`: cohort lifecycle states.
- `docs/findings/STREAK_LOYALTY_FINDINGS.md`: streak and recovery evidence.
- `docs/findings/KPI_DEEP_DIVE_FINDINGS.md`: funnel, revenue, inactivity, LTV, and acquisition.
